...
from routes.admin_routes import TranslationRequest, TTSRequest # Reuse existing models
...
# --- Shared Translation & Voice Utilities (Re-exposed for Parent Dashboard context) ---
...
async def parent_text_to_voice(req: TTSRequest):
    try:
        audio_base64 = parent_ai.generate_speech(req.text, req.language_code)
        return {\"status\": \"success\", \"audio_base64\": audio_base64}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.post(\"/voice-to-text\")
async def parent_voice_to_text(file: UploadFile = File(...), language_code: str = Form(\"en-US\")):
    try:
        audio_bytes = await file.read()
        transcription = parent_ai.process_audio_to_text(audio_bytes, language_code)
...
async def parent_audio_translator(
    ...
    try:
        audio_bytes = await file.read()
        result = parent_ai.audio_language_translator(audio_bytes, source_lang, target_lang)
